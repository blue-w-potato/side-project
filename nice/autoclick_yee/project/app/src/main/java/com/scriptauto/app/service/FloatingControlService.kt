...
class FloatingControlService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        scope.launch {
            try {
                ...
                if (script == null) {
                    showToast("找不到這個腳本(可能剛好被刪除了),請回列表重新啟動")
                    stopSelf(startId)
                    return@launch
                }
                when (val outcome = tryStart(script, buildResolver())) {
                    StartScriptOutcome.Started -> {
                        ...
                        showToast(outcomeMessage(outcome))
                        stopSelf(startId)
                    }
                }
            } catch (e: Exception) {
                showToast("啟動失敗:${e.message ?: e::class.simpleName}")
                stopSelf(startId)
            }
        }
    ...
    /**
     * 用 DisplayManager 抓螢幕尺寸,而不是 WindowManager.defaultDisplay——
     * Service 屬於「非 UI/非 Display」context,用 WindowManager 查詢畫面資訊在部分機型/版本上不可靠,
     * DisplayManager 才是官方建議在非 UI context 下取得顯示資訊的方式。
     */
