...
class EditorViewModel(
    ...
    /** 拖曳畫布圖示改變位置:用「當下最新位置 + 像素位移」計算,避免用到過時的閉包捕捉值 */
    fun applyDragDelta(uiId: Int, isDragStart: Boolean?, dxPx: Float, dyPx: Float, canvasWidthPx: Float, canvasHeightPx: Float) {
        val idx = items.indexOfFirst { it.uiId == uiId }
        if (idx == -1) return
        val current = items[idx].component
        val updated = when (current) {
            is ScriptComponent.LoopTap -> current.copy(position = current.position.shiftBy(dxPx, dyPx, canvasWidthPx, canvasHeightPx))
            is ScriptComponent.Hold -> current.copy(position = current.position.shiftBy(dxPx, dyPx, canvasWidthPx, canvasHeightPx))
            is ScriptComponent.Drag -> when (isDragStart) {
                true -> current.copy(start = current.start.copy(position = current.start.position.shiftBy(dxPx, dyPx, canvasWidthPx, canvasHeightPx)))
                false -> current.copy(end = current.end.copy(position = current.end.position.shiftBy(dxPx, dyPx, canvasWidthPx, canvasHeightPx)))
                null -> current
            }
            else -> return
        }
